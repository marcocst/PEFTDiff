...
from utils.utils import register_adaptation
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--sample_total_num", type=int, default=4)
    ...
    parser.add_argument("--wandb_project_name", type=str, default="subject-driven")

    # my config
    parser.add_argument("--rank", type=int, default=10, help=("The dimension of the LoRA update matrices."),)
    parser.add_argument("--transform_rank", type=int, default=1, help=("The dimension of the transform matrices."),)
    parser.add_argument("--alpha", type=float, default=1.0, help=("LoRA Scaling"),)
    parser.add_argument("--addition_part", type=str, default='lora', choices=['lora', 'ttv', 'ttm'])
...
def main(args):
    ...
    unet, td_layers = register_adaptation(
        unet,
        rank=args.rank,
        tensor_shape_dict=None,  # TODO
        transform_rank=args.transform_rank,
        alpha=args.alpha,
        addition_part=args.addition_part,
        use_rslora=True
    )
    ...
    optimizer = optim_class(
        td_layers.parameters(), **optim_args
    )
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        td_layers.parameters(),  # todo
                    )
                    accelerator.clip_grad_norm_(td_layers.parameters(), args.max_grad_norm)
